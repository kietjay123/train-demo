...
func _input(event) :
	...
	if event is InputEventKey :
		if event.keycode == KEY_F  && event.is_pressed() :
			...
			newUnit.position = Vector3(randi_range(250, 400), 6.5, randi_range(200, 300))
...
func mouseAction(button_pressed : int) -> void :
	...
	if button_pressed == 2 :
		...
		#var path : Curve3D = $Path3D.curve
		#for ii in result :
			#path.add_point(Vector3(ii.x, 0, ii.y))
...
func gather(squadNum : int ) -> Array[PackedVector2Array] :
	var result : Array[PackedVector2Array] = []
	var squadPos : Array[Vector3] = squadIndexer[str(squadNum)]["member"].map(func(input) : return input.position)
	print(squadPos)
	return result

func makeSquad(members : Array[Infantry]) -> void :
	pass
